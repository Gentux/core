...
func TestWinExe(sam string) {
	...
	cmd := exec.Command(nan.Config().Proxy.WinExe,
		"-U", "intra.nanocloud.com/Administrator%password",
		"--runas=intra.nanocloud.com/Administrator%password", "//10.20.12.10", //TODOHARDCODED
...
func (p *ProcCreateWinUser) Do() *nan.Err {
	...
	if bExecSecurityScript {
		...
		cmd := exec.Command(nan.Config().Proxy.WinExe,
			"-U", "intra.nanocloud.com/Administrator%password",
			"--runas=intra.nanocloud.com/Administrator%password", "//10.20.12.10", //TODO HARDCODED
			sWindowsServerSecurityFile)
		if out, err := cmd.Output(); err != nil {
